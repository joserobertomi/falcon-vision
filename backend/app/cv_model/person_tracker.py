...
class PersonDetector:
    """
    Person detection using YOLOv8 with WebSocket streaming support.
    
    This class can work in two modes:
    1. Standalone mode: Displays detections locally using OpenCV windows
    2. WebSocket mode: Streams detections to clients via ConnectionManager
    
    Example usage with ConnectionManager:
        ```python
        from app.connection_manager import manager
        from app.cv_model.person_tracker import PersonDetector
        
        # Inside a WebSocket endpoint
        @router.websocket("/detection/{camera_id}")
        async def detection_stream(websocket: WebSocket, camera_id: int, current_user: CurrentUserWS):
            client_id = str(current_user.id)
            await manager.connect(websocket, client_id)
            
            try:
                # Initialize detector with connection manager
                ...
                # Start detection (will stream to WebSocket)
    ...
    Example standalone usage:
        ```python
        # Without connection manager, displays locally
        detector = PersonDetector(camera_index=0, confidence_threshold=0.5)
        await detector.run_detection()
        ```
    """
    
    def __init__(self, camera_index=0, confidence_threshold=0.5, connection_manager=None, client_id: Optional[str] = None):
        """
        ...
        Args:
            ...
            connection_manager: ConnectionManager instance for WebSocket streaming
            client_id (str): Client ID to send frames to via WebSocket
        """
    ...
    async def send_frame_to_client(self, frame: np.ndarray) -> bool:
        """
        Encode frame and send it to the client via WebSocket.
        ...
        """
        if not self.connection_manager or not self.client_id:
            logger.warning("No connection manager or client_id configured")
            return False
        ...
        try:
            ...
            result = await self.connection_manager.send_frame(self.client_id, frame_bytes)
            return result
    ...
    async def run_detection(self):
        ...
        try:
            ...
            while True:
                ...
                # Send frame via WebSocket if connection manager is configured
                if self.connection_manager and self.client_id:
                    await self.send_frame_to_client(processed_frame)
                else:
                    # Display the frame locally if no WebSocket connection
